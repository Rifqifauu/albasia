...
use App\Filament\Resources\TallyLogResource\RelationManagers\PalletsRelationManager;
...
class TallyLogResource extends Resource
{
    ...
    protected static ?string $navigationLabel = 'Tally Log';
    ...
    public static function form(Form $form): Form
{
    ...
    ->color('success')
    ->url(fn ($record) => route('tallylog.download.pdf', $record)) // 👈 ganti jadi URL
    ->openUrlInNewTab(),
            ])
            ->filters([
                ...
                Filter::make('tanggal_range')
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        return $query
                            ->when(
                                $data['tanggal_dari'],
                                fn (Builder $query, $date): Builder => $query->whereDate('tally_log.created_at', '>=', $date),
                            )
                            ->when(
                                $data['tanggal_sampai'],
                                fn (Builder $query, $date): Builder => $query->whereDate('tally_log.created_at', '<=', $date),
                ...
                Filter::make('nomor_polisi')
                    ->form([
                        Select::make('nomor_polisi_filter')
                            ...
                            ->options(function () {
                                return TallyLog::select('nomor_polisi')
                                    ->distinct()
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        return $query->when(
                            ...
                            fn (Builder $query, $nopol): Builder => $query->where('tally_log.nomor_polisi', $nopol),
                        );
                    })
...
Tables\Actions\BulkAction::make('download_zip')
    ...
    public static function getPages(): array
    {
        return [
            ...
            'view' => Pages\ViewTallyLogs::route('/{record}/view'),
        ];
    }
}
